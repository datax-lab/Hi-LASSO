...
for data in dataset_names:    
    ...
    for iter_num in range(n_iter):
        ...
        relassoCV = RelaxedLassoCV(cv=5, verbose=False, normalize=True, fit_intercept=False)
        relassoCV.fit(X, y)                
        
        end_time = time.time()
        
        record.record_result(record_path, method, beta, beta_hat=relassoCV.coef_, exec_time=end_time-start_time)
